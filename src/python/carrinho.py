...
def BuscarVizinhos(no, mapa):
    ...
    for posicao in delta:
        ...
        if(VerificaLimites(mapa, novo_i, novo_j)
            and CelulaVazia(mapa, novo_i, novo_j)):
        ...
        if(vizinhos):
            ...
            for vizinho in vizinhos:
                ...
                if  (it ==  -1 or custo < caminho[it].f):
                    vizinho.f = custo
                    vizinho.anterior = No(i=atual.i,j=atual.j,
                        anterior=atual.anterior,f=atual.f)
...
def ListaMovimentos(caminho,destino):
    ...
    try:
        ...
        # Estipulando que o carrinho começa em
        #P, virado para o sul(FRENTE POWERBANK)
        for move in move_carrinho:
            
            # 1->2: Norte->Leste(direita)
            # 2->3: Leste->Sul(direita)
            # 3->4: sul->oeste(direita)
            ...
            # 3->2: Sul->leste(esquerda)
            # 2->1: Leste->Norte(esquerda)
            # 4->3: oeste->sul(esquerda)
...
def main():
    ...
    file = ['/home/pi/Documents/tcc_matheus/ A_Star_Search-main/src/python/mapa_5x5.csv']
